...
const InputField: React.FC<InputFieldProps> = ({label_text, input_type,input_value,input_ref, input_placeholder, triggerHanldler,optionalHandler,optional_text}) => {
  return (
    <div className='my-2'>
        ...
        <input ref={input_ref} className='w-full rounded-md focus:outline-none text-DT1 !bg-darkgray text-mediumgray p-[8px] border-[1px] border-bordergray placeholder:text-gray' type={input_type}  placeholder={input_placeholder} onChange={triggerHanldler} />
    </div>
  )
}
